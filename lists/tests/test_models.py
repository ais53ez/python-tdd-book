...
class ItemModelTest(TestCase):
    ...
    def test_duplicate_items_are_invalid(self):
        list_ = List.objects.create()
        duplicate_msg = 'Duplicate message'
        Item.objects.create(list=list_, text=duplicate_msg)
        with self.assertRaises(ValidationError):
            item = Item(list=list_, text=duplicate_msg)
            item.full_clean()
    ...
    def test_CAN_save_same_item_to_different_lists(self):
        ...
        duplicate_msg = 'Duplicate message'
        Item.objects.create(list=list1, text=duplicate_msg)
        item = Item(list=list2, text=duplicate_msg)
